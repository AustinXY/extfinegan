        ...
        for i in range(self.num_Ds):
            ...
            if i == 1: # Mutual information loss for the parent stage (1)
                ...
                for pt in range(cfg.NUM_PARTS):
                    temp_c_mk = torch.zeros_like(self.c_masked[pt]).cuda()
                    pti_code = torch.zeros([batch_size, cfg.NUM_PARTS]).cuda()
                    pti_code[:, pt] = 1

                    for ix in range(batch_size):
                        temp_c_mk[ix] = self.transform(self.c_masked[pt][ix].clone().cpu().detach()).cuda()

                    pred_pti = self.netsD[3](self.c_masked[pt])[0]
